...
```mermaid
flowchart TB
    A["用户输入"] --> B["对话完成 → 检查提取条件"]
    B --> C{"满足提取间隔？<br/>有新事实？<br/>不是闲聊？"}
    C -- 不满足 --> Z["本轮不提取"]
    C -- 满足 --> D["调用 LLM 提取事实"]
    D --> E["分类：资料 / 偏好 / 事件 / 知识 / 行为 / 技能"]
    E --> F{"与已有记忆是否矛盾？"}
    F -- 无矛盾 --> G["直接写入"]
    F -- 有矛盾 --> H["生成冲突候选<br/>等待用户确认"]
```
...
| 轴 | 轴类型 | 负端 / 低值 | 基线 / 中值 | 正端 / 高值 |
|----|--------|------------|------------|------------|
| 连接需求 | 单向轴（0 → 1） | 0.0 "保持距离" | 0.5 "正常互动" | 1.0 "渴望陪伴" |
| 沉浸度 | 单向轴（0 → 1） | 0.0 "心不在焉" | 0.5 "专注话题" | 1.0 "深度投入" |
| 防御感 | 中线轴（-1 ↔ 1） | -1.0 "完全信任" | 0.0 "中立基线" | 1.0 "高度谨慎" |
| 情绪基调 | 中线轴（-1 ↔ 1） | -1.0 "低落沮丧" | 0.0 "平静中性" | 1.0 "积极温暖" |
| 唤醒度 | 中线轴（-1 ↔ 1） | -1.0 "安静内敛" | 0.0 "正常活跃" | 1.0 "兴奋紧张" |
...
```mermaid
flowchart LR
    A["事件输入"] --> B["单轮上限<br/>每次变化不超过设定阈值"]
    B --> C["事件冷却<br/>同类型事件需间隔冷却"]
    C --> D["人格边界<br/>不同人格对情感敏感度不同"]
    D --> E["时间衰减<br/>长时间无交互后回归基线"]
```
...
flowchart TB
    R["涟漪情感系统（五轴状态）"]
    R --> P["人格枢控系统<br/>人格边界约束 · 情感敏感度 · 身份不改变"]
    R --> A["主动意识系统<br/>主动动机影响 · 行为选择偏好 · 用户输入优先"]
    R --> M["棱镜记忆系统<br/>情感事件不无条件写入长期记忆"]
    P --> V["语音合成系统<br/>音色选择 · 语速调整 · 情绪化表达"]
    A --> T["工具调用系统<br/>不干预执行 · 不绕过权限 · 事后评估"]
    M --> G["角色动画系统<br/>表情切换 · 姿态变化 · 动画活跃度"]
```
...
flowchart TB
    S["星图系统"]
    S --> M["记忆星图<br/>Memory Constellation"]
    S --> R["涟漪星图<br/>Ripple Constellation"]
    M --> M1["记忆对象节点 · 关系连线 · 来源消息追溯<br/>时间线导航 · 质量评估标记"]
    R --> R1["五轴状态实时快照 · 情绪事件时间线<br/>主动动机状态 · 语调指导参数 · 历史衰减曲线"]
    M1 --> E["Canvas 2D 渲染引擎<br/>PyQt5 WebEngine · 实时数据推送 5s"]
    R1 --> E
```
...
```mermaid
flowchart TB
    U["★ 用户<br/>（人物节点）"] -->|参与| P["★ 无人机项目<br/>（事件节点）"]
    J["★ 项目<br/>（项目节点）"] -->|使用| S["★ 视觉算法<br/>（技能节点）"]
    P --> S
    P -->|时间线| B["★ 基站测试<br/>（计划节点）"]
```
...
| 视图区域 | 显示内容（示意数据） |
|----------|----------------------|
| 五轴状态仪表盘 | 连接需求 0.72 · 沉浸度 0.58 · 防御感 -0.15 · 情绪基调 +0.42 · 唤醒度 +0.28 |
| 情绪事件时间线 | 温暖回复（+5min）· 工作协作（+12min）· 新计划（+28min）· 长时间离开（+1h）· 冲突（+2h） |
| 主动动机状态 | 连接需求：高 · 适合主动：是 · 冷却中：否 |
| 语调指导参数 | 语气：温暖 · 温度：偏高 · 活跃度：中等 |
